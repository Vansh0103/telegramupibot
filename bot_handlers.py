...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        return
    if not await enforce_access(update, ctx):
        return
...
async def admin_home(q, uid):
    ...
    kb = InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton(
                f"🔒 Force Join ({db.force_join_count(active_only=True)})",
                callback_data="admin_forcejoin",
            ),
            InlineKeyboardButton(
                "🛠 System", callback_data="admin_system"
            ),
        ],
        [
            InlineKeyboardButton(
    ...
    await q.edit_message_text(
        ...
        f"*Sessions:* {sessions}\n"
        f"*Banned:* {banned}\n"
        f"*Force Join:* {db.force_join_count(active_only=True)} channels\n"
        f"*Maintenance:* {'ON' if system_setting_bool("maintenance_mode", False) else 'OFF'}\n\n"
        f"*Plans:*\n"
        f"🆓 Free: {free_cnt}\n"
        f"⭐ Premium: {prem_cnt}\n"
        f"👑 VIP: {vip_cnt}\n\n"
...
async def admin_forcejoin_menu(q, uid):
    rows = db.get_force_join_channels(active_only=False)
    enabled = system_setting_bool("force_join_enabled", True)
    if not rows:
        body = "_No force-join channels configured yet._"
    else:
        body = "\n".join(format_force_join_channel(r) for r in rows[:25])
        if len(rows) > 25:
            body += f"\n\n_... and {len(rows) - 25} more_"

    kb = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("➕ Add Channel", callback_data="admin_forcejoin_add"),
            InlineKeyboardButton("➖ Remove", callback_data="admin_forcejoin_remove"),
        ],
        [
            InlineKeyboardButton(
                f"{'🟢' if enabled else '🔴'} Toggle Force Join",
                callback_data="admin_forcejoin_toggle",
            ),
            InlineKeyboardButton("🔄 Refresh", callback_data="admin_forcejoin"),
        ],
...
async def admin_system_menu(q, uid):
    maintenance = system_setting_bool("maintenance_mode", False)
    fj_enabled = system_setting_bool("force_join_enabled", True)
    fj_count = db.force_join_count(active_only=True)
    text = (
        f"🛠️ *System Controls*\n\n"
        f"Maintenance: *{'ON' if maintenance else 'OFF'}*\n"
        f"Force Join: *{'ON' if fj_enabled else 'OFF'}*\n"
        f"Required Channels: *{fj_count}*\n"
    ...
    kb = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                ...
                callback_data="admin_maint_toggle",
            ),
        ],
        [
            InlineKeyboardButton("🔒 Force Join Manager", callback_data="admin_forcejoin"),
        ],
        [InlineKeyboardButton("◀️ Back", callback_data="admin_home")],
    ])
...
async def on_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "forcejoin_refresh":
        if not await enforce_access(update, ctx, allow_force_refresh=True):
            return
        await q.answer("Access granted!", show_alert=False)
    ...
    if data == "admin_forcejoin":
        if not is_admin(uid):
            return
        await admin_forcejoin_menu(q, uid)
        return

    if data == "admin_forcejoin_add":
        if not is_admin(uid):
            return
        await ask_state(
            q, ctx, ST_ADMIN_FORCEJOIN_ADD,
            "🔒 *Add Force-Join Channel*\n\n"
            "Send one of these formats:\n"
            "`@channelusername`\n"
            "`https://t.me/channelusername`\n"
            "`-1001234567890 | https://t.me/+invite | Channel Title`\n\n"
            "For private channels, add the chat ID and join link.",
        )
        return

    if data == "admin_forcejoin_remove":
        if not is_admin(uid):
            return
        rows = db.get_force_join_channels(active_only=False)
        body = "\n".join(format_force_join_channel(r) for r in rows[:20]) if rows else "_No channels saved._"
        await ask_state(
            q, ctx, ST_ADMIN_FORCEJOIN_REMOVE,
            "➖ *Remove Force-Join Channel*\n\n"
            "Send the channel row ID, chat ref, title, or link to remove.\n\n"
            f"{body}",
        )
        return

    if data == "admin_forcejoin_toggle":
        if not is_admin(uid):
            return
        cur = system_setting_bool("force_join_enabled", True)
        db.set_setting(0, "force_join_enabled", "false" if cur else "true")
        await admin_forcejoin_menu(q, uid)
        return
    ...
    if data == "admin_system":
        if not is_admin(uid):
            return
        await admin_system_menu(q, uid)
        return

    if data == "admin_maint_toggle":
        if not is_admin(uid):
            return
        cur = system_setting_bool("maintenance_mode", False)
        db.set_setting(0, "maintenance_mode", "false" if cur else "true")
        await admin_system_menu(q, uid)
        return
...
def normalize_phone_input(phone: str) -> str:
    """Normalize phone input so users can send it in flexible formats."""
    raw = (phone or "").strip()
    if not raw:
        return ""
...
def normalize_otp_input(code: str) -> str:
    """Normalize OTP input so users can paste spaces/dashes/etc."""
    raw = (code or "").strip()
    if not raw:
        return ""
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ═══════════ ADMIN: FORCE JOIN ADD ═══════════

    if state == ST_ADMIN_FORCEJOIN_ADD:
        if not is_admin(uid):
            ctx.user_data.clear()
            return

        chat_ref, join_url, title = parse_force_join_input(text)
        if not chat_ref:
            await update.message.reply_text(
                "❌ Invalid format. Send @username, t.me link, or `chat_id | join_link | title`.",
                parse_mode="Markdown",
            )
            return

        db.add_force_join_channel(chat_ref, join_url, title, uid)
        db.log(uid, "force_join_add", f"{chat_ref} | {join_url or ''}", "admin")
        ctx.user_data.clear()
        rows = db.get_force_join_channels(active_only=False)
        body = "\n".join(format_force_join_channel(r) for r in rows[:25])
        await update.message.reply_text(
    ...
    # ═══════════ ADMIN: FORCE JOIN REMOVE ═══════════

    if state == ST_ADMIN_FORCEJOIN_REMOVE:
        if not is_admin(uid):
            ctx.user_data.clear()
            return

        removed = db.remove_force_join_channel(text.strip())
        if not removed:
            await update.message.reply_text(
                "❌ Channel not found. Send the row ID, chat ref, title, or link exactly as listed.",
            )
            return

        db.log(uid, "force_join_remove", text.strip(), "admin")
        ctx.user_data.clear()
        rows = db.get_force_join_channels(active_only=False)
        body = "\n".join(format_force_join_channel(r) for r in rows[:25]) if rows else "_No channels left._"
        await update.message.reply_text(
            f"✅ Force-join channel removed.\n\n🔒 *Current Channels:*\n{body}",
            parse_mode="Markdown",
            reply_markup=main_kb(uid),
            disable_web_page_preview=True,
        )
